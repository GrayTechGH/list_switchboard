...
class UrlFetcherDublinLiteraryAward(UrlFetcherGeneric):

  FILTER_CATEGORIES = (
    CATEGORY_LITERARY_GENERAL_FICTION,
    CATEGORY_REGIONAL_NATIONAL_AWARDS,
  )
  source_id = 'dublin_literary_award'
  NAME = 'Dublin Literary Award'
  URL = OFFICIAL_URL
  ...
  def create_official_parser(self):
    try:
      from calibre_plugins.list_switchboard.parser.dublin_literary_award import (
        DublinLiteraryAwardOfficialParser,
      )
    except ImportError:
      from parser.dublin_literary_award import DublinLiteraryAwardOfficialParser
    return DublinLiteraryAwardOfficialParser()

  def create_wikipedia_parser(self):
    try:
      from calibre_plugins.list_switchboard.parser.dublin_literary_award import (
        DublinLiteraryAwardWikipediaParser,
      )
    except ImportError:
      from parser.dublin_literary_award import DublinLiteraryAwardWikipediaParser
    return DublinLiteraryAwardWikipediaParser()

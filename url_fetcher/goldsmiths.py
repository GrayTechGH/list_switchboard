...
class UrlFetcherGoldsmithsPrize(UrlFetcherGeneric):

  FILTER_CATEGORIES = (
    CATEGORY_LITERARY_GENERAL_FICTION,
    CATEGORY_REGIONAL_NATIONAL_AWARDS,
  )
  source_id = 'goldsmiths_prize'
  ...
  def create_official_parser(self):
    try:
      from calibre_plugins.list_switchboard.parser.goldsmiths import GoldsmithsOfficialParser
    except ImportError:
      from parser.goldsmiths import GoldsmithsOfficialParser
    return GoldsmithsOfficialParser()

  def create_wikipedia_parser(self):
    try:
      from calibre_plugins.list_switchboard.parser.goldsmiths import GoldsmithsWikipediaParser
    except ImportError:
      from parser.goldsmiths import GoldsmithsWikipediaParser
    return GoldsmithsWikipediaParser()
